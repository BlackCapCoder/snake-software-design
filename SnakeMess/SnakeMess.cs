...
new Diagnostics.Stopwatch();t.Start();Clear();CursorVisible=!A;Title="snk";S:do{p.X=r.Next(0,w);p.Y=r.Next(0,h);}while(s
...
(b^=k==32)|t.ElapsedMilliseconds<100)goto C;t.Restart();G(new[]{s.Last(),n,s.First()},14,"0@ ");if(n!=p)s.RemoveAt(0);s.
Add(n);if(n==p)goto S;}void G(P[]j,int c,string x){ForegroundColor=(ConsoleColor)c;for(v=0;v<x.Length;){SetCursorPosition
(j[v].X,j[v].Y);Write(x[v++]);}}}}}
