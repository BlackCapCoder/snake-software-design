...
int)ReadKey().Key:0;v<4;)if(k==40-(v*7)%4&a!=v++)a=++v%4;P q=s.Last(),n=new P(q.X-(--a*3-1)%2,q.Y+(++a*3-1)%2);A=k!=27^s
.Contains(n);Threading.Thread.Sleep(80);if(b^=k==32)goto C;G(new[]{q,n,s.First()},"0@ ",33);s=s.Skip(n!=p?1:0).Append(n)
;if(n==p)goto S;}void G(P[]j,string x,int o){v=0;foreach(var c in x)Write($"\x1B[{o}m\x1B[{j[v].Y};{j[v++].X}H"+c);}}}}
