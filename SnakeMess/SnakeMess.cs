...
X-((a-1)*3-1)%2,Y=s.Last().Y+(a*3-1)%2};A=k!=27&n.X>=0&n.X<w&n.Y>=0&n.Y<h&!s.Contains(n);if((b^=k==32)|(T.Now-t).
Milliseconds<100)goto C;t=T.Now;v=14;G(new[]{s.Last(),n,s.First()},"0@ ");s=s.Skip(n!=p?1:0).Append(n);if(n==p)goto S;}
void G(P[]j,string x){ForegroundColor=(ConsoleColor)v;for(v=0;v<x.Length;){SetCursorPosition(j[v].X,j[v].Y);Write(x[v++]
);}}}}}
